...
ecapture tls -m [text|keylog|pcap|har] [flags] [pcap filter expression (for pcap mode)]
...
ecapture tls -m har --har-endpoint=http://host:port/adr/har --har-filter-host=baidu.com --pid=1234
...
func init() {
	...
	opensslCmd.PersistentFlags().StringVarP(&opensslConfig.CaptureMode, "model", "m", "text", "capture model, such as : text, pcap/pcapng, key/keylog, har")
	...
	opensslCmd.PersistentFlags().StringVar(&opensslConfig.HarEndpoint, "har-endpoint", "", "HTTP endpoint URL to POST HAR entries (for har mode), e.g: http://host:port/adr/har")
	opensslCmd.PersistentFlags().StringVar(&opensslConfig.HarFilterHost, "har-filter-host", "", "Filter captured traffic by HTTP Host header (for har mode), e.g: baidu.com")
